let greeting:string; // Declared variable "greeting" with string value annotation 
...
let random:string = "Alpha"; // creating string variable
...
!aa; !bb; !cc; // NotGate expression (Problem #7 Solved)
...
let score:number = 45; // Grading System
let grade:string;  // string variable to assign grade
...
let x = 23; let y = 23; let z = 44; //Max of three
if (x>=y && x>=z){
...
let leapYear:boolean = (year%4 === 0) && (year%100 !== 0 || year%400 ===0); // checking if the given year is divisible by 4 and not by 100, if it is then it must also be divisible by 400
if(leapYear = true){
    console.log(`${year} is a leap year`)// logging the result (Problem #13 solved)
}
